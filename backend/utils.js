...
const getStartTimeAndEndTime = (meetingTime, meetingDate, durationInMinutes) => {
    const dateTimeString = `${meetingDate} ${meetingTime}`;

    const startDateTime = DateTime.fromFormat(dateTimeString, 'yyyy-MM-dd h:mm', {zone: 'Africa/Nairobi'});
...
const getCalendarId = async (calendar, calendarSummary, google_auth) => {
    ...
    let targetCalendar = calendarList.data.items.find(cal => cal.summary === calendarSummary);
...
module.exports = {
    getCalendarId, sendMail, getStartTimeAndEndTime
